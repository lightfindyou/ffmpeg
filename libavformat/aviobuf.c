...
{
    DEBUG("buffer:%p , buf_ptr:%p, buf_end:%p, data:%p, len:%d\n",
        s->buffer, s->buf_ptr, s->buf_end, data, len);
    if (!s->error) {
        int ret = 0;
...
void avio_write(AVIOContext *s, const unsigned char *buf, int size)
{
    ...
    while (size > 0) {
        ...
        DEBUG("trace memcpy from:%p, to %p, len:%d\n", s->buf_ptr, buf, len);
#if USETS_MEMCPY
//xzjin
...
static void fill_buffer(AVIOContext *s)
{
    ...
    if (len == AVERROR_EOF) {
        ...
        s->pos += len;
        s->buf_ptr = dst;   //xzjin 设置成buf_ptr
        s->buf_end = dst + len;
        s->bytes_read += len;
    }
}
...
#endif  // USETS_MEMCPY
            DEBUG("memcpy from %p, to %p, len: %d.\n", s->buf_ptr, buf, len);
            buf += len;
            s->buf_ptr += len;
            size -= len;
        }
...
#define MPROTECT_MAX   (30*1024)   //30K
    ...
    if (!buffer)
        return AVERROR(ENOMEM);
    DEBUG("Buffer from:%p, to:%p\n", buffer, buffer+buffer_size);
//    {
//    void* protectBegin;
//    int protectLen, ret, ern = 0;
//    protectBegin = (((unsigned long)buffer>>12)+1)<<12;
//    protectLen = (((unsigned long)(buffer+buffer_size)>>12)<<12)-(unsigned long)protectBegin;
//    protectLen = protectLen<MPROTECT_MAX?protectLen:MPROTECT_MAX;
//    if(protectLen>MPROTECT_MAX){
//        DEBUG("Protect begin:%p, protect len:%d\n", protectBegin, protectLen);
//        ret = mprotect(protectBegin, protectLen, PROT_READ);
//        ern = errno;
//        if(!ret){
//            DEBUG("Mprotect memory from:%p, len:%d.\n", protectBegin, protectLen);
//        }else{
//            DEBUG("Failed to mprotect memory, errno:%d.\n%s \n", ern, strerror(ern));
//            goto fail;
//        }
//    }else{
//        DEBUG("NOT protect len:%d, buf length:%d\n", protectLen, buffer_size);
//    }
//    }
    //xzjin 这里的AVIOContext是output_files里面的ctx指向的
...
#endif  //USETS_MEMCPY
        DEBUG("memcpy from %p to %p, size:%d\n", buf + buf_size,
...
//xzjin 这里释放AVIOContext,释放output_files->ctx时下需要调用这里
