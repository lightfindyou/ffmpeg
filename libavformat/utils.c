 ...
 * Return the number of bytes read or an error. */ //xzjin read data to pkt->data
...
#endif
/* Open input file and probe the format if necessary. */    //xzjin 打开文件
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    s->probe_score = ret;
    {
        AVIOContext *pb = s->pb;
        if(pb){
            DEBUG("After input pb:%p, pb->buffer: %p, pb->buffer_size: %d, pb->buf_ptr: %p, pb->buf_end: %p\n",
                 pb, pb->buffer, pb->buffer_size, pb->buf_ptr, pb->buf_end);
        }else{
            DEBUG("pb NULL\n");
        }
    }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    for (;;) {
        ...
        if (!pktl && st->internal->request_probe <= 0)
            return ret;
        //xzjin 这里把packet放到了raw_packet_buffer链表里面
                        ...
                        int stream_index, int flush)
{
    ...
    DEBUG("pkt %p :pkt->data:%p\n", pkt, pkt?pkt->data:NULL);
    ...
    while (size > 0 || (flush && got_output)) {
        ...
        DEBUG("After parse_packet out_pkt %p :out_pkt->data:%p\n", out_pkt, out_pkt?out_pkt->data:NULL);
        ...
        DEBUG("out_pkt %p :out_pkt->data:%p\n", out_pkt, out_pkt?out_pkt->data:NULL);
...
static int read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while (!got_packet && !s->internal->parse_queue) {
        ...
        DEBUG("After read_packet pkt %p :pkt->data:%p\n", pkt, pkt?pkt->data:NULL);
        if (ret < 0) {
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!st->need_parsing || !st->parser) {
            ...
            if ((ret = parse_packet(s, pkt, pkt->stream_index, 0)) < 0)
                return ret;
            DEBUG("After parse_packet pkt %p :pkt->data:%p\n", pkt, pkt?pkt->data:NULL);
    ...
    if (!got_packet && s->internal->parse_queue){
        ...
        DEBUG("After avpriv_packet_list_get pkt %p :pkt->data:%p\n", pkt, pkt?pkt->data:NULL);
    }
...
int av_read_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (!genpts) {
        chooser = s->internal->packet_buffer;
        ret = s->internal->packet_buffer
              ? avpriv_packet_list_get(&s->internal->packet_buffer,
                                        &s->internal->packet_buffer_end, pkt)
              : read_frame_internal(s, pkt);
        DEBUG("buffer or read pkt %s : %p, pkt->data:%p\n", 
            chooser?"buffered read":"read internal", pkt, pkt?pkt->data:NULL);
        if (ret < 0)
            return ret;
        goto return_packet;
    }
        ...
        if (pktl) {
            ...
            if (!(next_pkt->pts == AV_NOPTS_VALUE && st->discard < AVDISCARD_ALL &&
                ...
                ret = avpriv_packet_list_get(&s->internal->packet_buffer,
                                               &s->internal->packet_buffer_end, pkt);
                DEBUG("avpriv_packet_list_get pkt: %p, pkt->data:%p\n", pkt, pkt?pkt->data:NULL);
                goto return_packet;
            }
        }

        ret = read_frame_internal(s, pkt);
        DEBUG("read_frame_internal pkt: %p, pkt->data:%p\n", pkt, pkt?pkt->data:NULL);
        if (ret < 0) {
            if (pktl && ret != AVERROR(EAGAIN)) {
                eof = 1;
                continue;
