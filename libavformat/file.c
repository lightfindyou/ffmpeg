...
//    ret = ts_read(c->fd, buf, size);
    if(extraWriteFd == 0){
        extraWriteFd = open("./target.txt", O_RDWR|O_CREAT, S_IRUSR|S_IWUSR);
        if(extraWriteFd <0){
    ...
    //MSG("Bytes cur read:%d, totally read: %lu.\n", ret, totalRead);
#else
    ret = read(c->fd, buf, size);
#endif
    DEBUG("trace read to buf:%p, buf end:%p, size:%d\n", buf, buf+size, size);
    if (ret == 0 && c->follow)
