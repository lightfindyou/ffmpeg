...
#include "internal.h"
#include "packet.h"
#include "../libavformat/debug_xzjin.h"
...
int av_grow_packet(AVPacket *pkt, int grow_by)
{
    ...
    if (pkt->buf) {
        ...
        if (new_size + data_offset > pkt->buf->size ||
            ...
            //DEBUG("relocate pkt->buf->data to %p\n" ,NULL , pkt->buf->data);
            if (ret < 0) {
                pkt->data = old_data;
                return ret;
            }
...
int av_packet_make_refcounted(AVPacket *pkt)
{
    ...
    if (pkt->size){
        ...
        memcpy(pkt->buf->data, pkt->data, pkt->size);
#endif  // USETS_MEMCPY
        DEBUG("trace memcpy from %p to %p, size %d\n",
             pkt->data, pkt->buf->data, pkt->size);
    }
