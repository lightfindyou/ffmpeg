...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost, int unqueue)
{
    ...
    AVStream *st = ost->st;
    int ret;
    DEBUG("pb:%p, pb->buf_ptr:%p\n", s->pb, s->pb?s->pb->buf_ptr:NULL);
                          ...
                          OutputStream *ost, int eof)
{
    ...
    if(of && of->ctx){
        AVFormatContext *s = of->ctx;
        DEBUG("pb:%p, pb->buf_ptr:%p, pkt->data:%p, pkt->bufRef:%p, pkt->bufRef->data:%p\n",
             s->pb, s->pb?s->pb->buf_ptr:NULL, pkt->data, pkt->buf, pkt->buf->data);
    }else{
        DEBUG("of or of->ctx NULL.\n");
    }
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
    ...
    if(of && of->ctx){
        AVFormatContext *s = of->ctx;
        DEBUG("ost->file_index:%d\n", ost->file_index);
        DEBUG("trace AVIOContext AKA pb:%p, pb->buf_ptr:%p\n", s->pb, s->pb?s->pb->buf_ptr:NULL);
    }else{
        DEBUG("of or of->ctx NULL.\n");
    }
